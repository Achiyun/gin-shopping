...
func (con GoodsCateController) Add(c *gin.Context) {
	//获取顶级分类
	goodsCateList := []models.GoodsCate{}
	global.GVA_DB.Where("pid = 0").Find(&goodsCateList)
...
func (con GoodsCateController) DoAdd(c *gin.Context) {
	...
	pid, err1 := utils.Int(c.PostForm("pid"))
	...
	err := global.GVA_DB.Create(&goodsCate).Error
	if err != nil {
...
func (con GoodsCateController) DoEdit(c *gin.Context) {
	...
	title := c.PostForm("title")
	pid, err2 := utils.Int(c.PostForm("pid"))
	link := c.PostForm("link")
	template := c.PostForm("template")
	subTitle := c.PostForm("sub_title")
	keywords := c.PostForm("keywords")
	...
	err := global.GVA_DB.Save(&goodsCate).Error
	if err != nil {
		con.Error(c, "修改失败", "/admin/goodsCate/edit?id="+utils.String(id))
		return
	}
...
func (con GoodsCateController) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		if goodsCate.Pid == 0 { //顶级分类
			goodsCateList := []models.GoodsCate{}
			global.GVA_DB.Where("pid = ?", goodsCate.Id).Find(&goodsCateList)
			if len(goodsCateList) > 0 {
				con.Error(c, "当前分类下面子分类，请删除子分类作以后再来删除这个数据", "/admin/goodsCate")
			} else {
				global.GVA_DB.Delete(&goodsCate)
