...
func (con AccessController) Add(c *gin.Context) {
	//获取顶级模块
	accessList := []models.Access{}
	global.GVA_DB.Where("module_id=?", 0).Find(&accessList)
...
func (con AccessController) DoAdd(c *gin.Context) {
	...
	accessType, err1 := utils.Int(c.PostForm("type"))
	url := c.PostForm("url")
	moduleId, err2 := utils.Int(c.PostForm("module_id"))
	...
	err5 := global.GVA_DB.Create(&access).Error
	if err5 != nil {
...
func (con AccessController) DoEdit(c *gin.Context) {
	...
	moduleName := strings.Trim(c.PostForm("module_name"), " ")
	actionName := c.PostForm("action_name")
	accessType, err2 := utils.Int(c.PostForm("type"))
	url := c.PostForm("url")
	moduleId, err3 := utils.Int(c.PostForm("module_id"))
	sort, err4 := utils.Int(c.PostForm("sort"))
	status, err5 := utils.Int(c.PostForm("status"))
	description := c.PostForm("description")
	...
	if moduleName == "" {
		con.Error(c, "模块名称不能为空", "/admin/access/edit?id="+utils.String(id))
		return
	}
	...
	err := global.GVA_DB.Save(&access).Error
	if err != nil {
		con.Error(c, "修改数据", "/admin/access/edit?id="+utils.String(id))
	} else {
		con.Success(c, "修改数据成功", "/admin/access/edit?id="+utils.String(id))
	}
...
func (con AccessController) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		if access.ModuleId == 0 { //顶级模块
			accessList := []models.Access{}
			global.GVA_DB.Where("module_id = ?", access.Id).Find(&accessList)
			if len(accessList) > 0 {
				con.Error(c, "当前模块下面有菜单或者操作，请删除菜单或者操作以后再来删除这个数据", "/admin/access")
			} else {
				global.GVA_DB.Delete(&access)
