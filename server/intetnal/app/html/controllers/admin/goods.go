...
func (con GoodsController) Index(c *gin.Context) {
	...
	global.GVA_DB.Where(where).Offset((page - 1) * pageSize).Limit(pageSize).Find(&goodsList)

	//获取总数量
	var count int64
	global.GVA_DB.Where(where).Table("goods").Count(&count)
...
func (con GoodsController) Add(c *gin.Context) {
	...
	global.GVA_DB.Where("pid=0").Preload("GoodsCateItems").Find(&goodsCateList)

	//获取所有颜色信息
	goodsColorList := []models.GoodsColor{}
	global.GVA_DB.Find(&goodsColorList)
...
func (con GoodsController) GoodsTypeAttribute(c *gin.Context) {
	cateId, err1 := utils.Int(c.Query("cateId"))
	goodsTypeAttributeList := []models.GoodsTypeAttribute{}
	err2 := global.GVA_DB.Where("cate_id = ?", cateId).Find(&goodsTypeAttributeList).Error
...
func (con GoodsController) DoAdd(c *gin.Context) {
	...
	cateId, _ := utils.Int(c.PostForm("cate_id"))
	goodsNumber, _ := utils.Int(c.PostForm("goods_number"))
	//注意小数点
	marketPrice, _ := utils.Float(c.PostForm("market_price"))
	price, _ := utils.Float(c.PostForm("price"))
	...
	isDelete, _ := utils.Int(c.PostForm("is_delete"))
	isHot, _ := utils.Int(c.PostForm("is_hot"))
	isBest, _ := utils.Int(c.PostForm("is_best"))
	isNew, _ := utils.Int(c.PostForm("is_new"))
	goodsTypeId, _ := utils.Int(c.PostForm("goods_type_id"))
	...
	go func() {
		...
		for _, v := range goodsImageList {
			...
			global.GVA_DB.Create(&goodsImgObj)
		}
		wg.Done()
	}()
	//6、增加规格包装
	...
	go func() {
		...
		for i := 0; i < len(attrIdList); i++ {
			goodsTypeAttributeId, attributeIdErr := utils.Int(attrIdList[i])
			if attributeIdErr == nil {
				//获取商品类型属性的数据
				goodsTypeAttributeObj := models.GoodsTypeAttribute{Id: goodsTypeAttributeId}
				global.GVA_DB.Find(&goodsTypeAttributeObj)
				//给商品属性里面增加数据  规格包装
				...
				global.GVA_DB.Create(&goodsAttrObj)
			}
...
func (con GoodsController) Edit(c *gin.Context) {
	...
	for i := 0; i < len(goodsColorList); i++ {
		if _, ok := goodsColorMap[utils.String(goodsColorList[i].Id)]; ok {
	...
	global.GVA_DB.Where("goods_id=?", goods.Id).Find(&goodsImageList)
	...
	global.GVA_DB.Where("goods_id=?", goods.Id).Find(&goodsAttr)
	goodsAttrStr := ""
	...
	for _, v := range goodsAttr {
		if v.AttributeType == 1 {
			...
			global.GVA_DB.Find(&goodsTypeArttribute)
...
func (con GoodsController) DoEdit(c *gin.Context) {
	...
	err3 := global.GVA_DB.Save(&goods).Error
	if err3 != nil {
		con.Error(c, "修改失败", "/admin/goods/edit?id="+utils.String(id))
		return
	}
	...
	// 6.1删除当前商品下面的规格包装
	goodsAttrObj := models.GoodsAttr{}
	global.GVA_DB.Where("goods_id=?", goods.Id).Delete(&goodsAttrObj)
	//6.2、重新执行增加
	wg.Add(1)
	go func() {
		attrIdList := c.PostFormArray("attr_id_list")
...
func (con GoodsController) ChangeGoodsImageColor(c *gin.Context) {
	//获取图片id 获取颜色id
	goodsImageId, err1 := utils.Int(c.Query("goods_image_id"))
	colorId, err2 := utils.Int(c.Query("color_id"))
	goodsImage := models.GoodsImage{Id: goodsImageId}
	global.GVA_DB.Find(&goodsImage)
	goodsImage.ColorId = colorId
	err3 := global.GVA_DB.Save(&goodsImage).Error
...
func (con GoodsController) RemoveGoodsImage(c *gin.Context) {
	...
	goodsImageId, err1 := utils.Int(c.Query("goods_image_id"))
	goodsImage := models.GoodsImage{Id: goodsImageId}
	err2 := global.GVA_DB.Delete(&goodsImage).Error
	if err1 != nil || err2 != nil {
		c.JSON(http.StatusOK, gin.H{
			"result":  "删除失败",
			"success": false,
